...
void fitTo_minv_unbinned(){
  ...
  double CostValues[23] = {-1.00,-0.80,-0.60,-0.50,-0.40,-0.30,-0.20,-0.16,-0.12,-0.08,-0.04,0.00,0.04,0.08,0.12,0.16,0.20,0.30,0.40,0.50,0.60,0.80,1.00};
  double PhiValues[11] = {0.000000,0.502655,1.005310,1.256637,1.445133,1.570796,1.696460,1.884956,2.136283,2.638938,3.141593};
  ...
  for(int i = 5;i < 6;i++){
    ...
    for(int j = 5;j < 6;j++){
      ...
      // Drawing the plot ...
      //============================================================================
      RooPlot *DimuMass_unb_frame = DimuMass_unb.frame(Title("Dimuon mass distribution"));
      dataset.plotOn(DimuMass_unb_frame);
      model.plotOn(DimuMass_unb_frame,Components("bck_VWG"),LineStyle(kDashed),LineColor(kGray+1));
      model.plotOn(DimuMass_unb_frame,Components("sig_CB2"),DrawOption("F"),FillColor(kRed+1),FillStyle(3344));
      model.plotOn(DimuMass_unb_frame,Components("sig_CB2"),DrawOption("L"),LineColor(kRed+1));
